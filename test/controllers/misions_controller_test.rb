...
class MisionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create mision" do
    assert_difference('Mision.count') do
      post misions_url, params: { mision: {  title: @mision. title, priority: @mision.priority, status: @mision.status } }
    end
  ...
  test "should update mision" do
    patch mision_url(@mision), params: { mision: {  title: @mision. title, priority: @mision.priority, status: @mision.status } }
